...
class Agent():
    ...
    def get_neighbour_blocks(self):
        ...
        for l in range(l_start, l_start+self.nb_bloc_neighb):
            for c in range(c_start, c_start+self.nb_bloc_neighb):
                try:
                    if self.terrain.isFull((l,c)):  #map infranchissalbe à cet endroit
                        topleft = c*self.terrain.bloc_size, l*self.terrain.bloc_size
                        blocks.append(pygame.Rect((topleft), (self.terrain.bloc_size, self.terrain.bloc_size)))
    ...
    def bouge(self):
        ...
        """
        #rectangle Pygame qui encadre la zone infranchissable de l'agent
        self.rect = pygame.Rect(self.pos[0]+self.bloc_delta,  
        ...
        for rect in blocks: #dessins de rectangle rouge sur les blocs en collision
            pygame.draw.rect(self.terrain.screen, (255,0,0), rect,3)
        pygame.draw.rect(self.terrain.screen, (0,0,255), self.rect, 3)
        ...
        #position modifiée selon vitesse de l'agent, sans que la zone infranchissable déborde du cadre
        if self.v !=0 :
            x = self.pos[0] - self.v*self.l_sin[self.l_rotation[0]]
            y = self.pos[1] - self.v*self.l_cos[self.l_rotation[0]]
            if x+self.bloc_delta < 0: # la zone infranchissable sort sur la gauche
                x, self.v, self.accel = -self.bloc_delta,0,0
            elif (x+self.bloc_delta+self.bloc_size) > self.terrain.nb_pixelsX: # zone infranchissable sort sur la droite
                x, self.v, self.accel = self.terrain.nb_pixelsX-self.bloc_delta-self.bloc_size,0,0
            if y+self.bloc_delta<0:  # zone infranchissable sort en haut
                y, self.v, self.accel = -self.bloc_delta,0,0
            elif (y+self.bloc_delta+self.bloc_size) > self.terrain.nb_pixelsY:  # zone infranchissable sort en bas
                y, self.v, self.accel = self.terrain.nb_pixelsY-self.bloc_delta-self.bloc_size,0,0
            self.pos = (x,y)
